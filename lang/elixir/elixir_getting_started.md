...
[Create and use a supervisor](https://github.com/arafatm/notes/commit/714ebd3)

Create `KV.Supervisor`

Update `KV.Registry` to use name

Update `KV.RegistryTest` with new `start_link/1` update

Experiment with `iex -S mix`

```elixir
iex> KV.Supervisor.start_link                       # {:ok, #PID<0.66.0>}
iex> KV.Registry.create(KV.Registry, "shopping")    # :ok
iex> KV.Registry.lookup(KV.Registry, "shopping")    # {:ok, #PID<0.70.0>}
```

#### Understanding applications

`iex -S mix` == `iex -S mix start`
- `iex -S mix run --no-start` => we have to `Application.start(:kv)` to use it

[Application.start callback](https://github.com/arafatm/notes/commit/4b65901) 
defines how to start the application. With this change, 
`KV.Supervisor.start_link` is automatically called when started with `iex -S 
mix`

#### Supervisor restarting Buckets

[Test to see what happens when bucket 
crashes](https://github.com/arafatm/notes/commit/892081f)
...
To resolve we define a supervisor that will spawn and supervise all buckets.  
The strategy used `:simple_one_for_one` specified a worker template and 
supervises many children

#### Supervision trees

[Create a bucket supervisor](https://github.com/arafatm/notes/commit/3c62fc5)

[Make KV.Bucket.Supervisor a child of 
KV.Supervisor](https://github.com/arafatm/notes/95e502e)

Strategies
- `:one_for_one`
- `:one_for_all` supervisor will kill and restart all children procs
- `:rest_for_one` supervisor will only kill and restart child procs started 
  after the crashed child

[Use :rest_for_one strategy](https://github.com/arafatm/notes/a7adef2)

#### Observer

In `iex -S mix` we can run a gui observer with `iex> :observer.start`

If we create a new proc e.g. `KV.Registry.create KV.Registry, "shopping"` we 
will see it in the Observer
...
When accessing buckets we need to message the registry *which becomes a 
bottleneck*

**ETS** can be used as a cache

:boom: Do not prematurely optimize with ETS. Analyze app performance to 
identify bottlenecks

```elixir
iex> table = :ets.new(:buckets_registry, [:set, :protected])      # 8207
iex> :ets.insert(table, {"foo", self()})                          # true
iex> :ets.lookup(table, "foo")                        # [{"foo", #PID<0.41.0>}]
```

- args: table name, options
- `:set` => no dupe keys
- `:protected` => only process that created table can writ it. All procs read
- passing `[:named_table]` returns an atom ":bucket_registry" as name

[Code changes to use ETS](https://github.com/arafatm/notes/commit/6a22505)

:boom: tests still fail e.g. on `test "spawns buckets"`

#### Race conditions?

Failing tests are due to delay between operation and time to change in ETS
- We are using `cast` instead of `call` so it's synchronous

[Code changes to handle_call](https://github.com/arafatm/notes/commit/818d037)
